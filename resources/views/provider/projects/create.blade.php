    ...
    <div class="py-10">
        <div class="max-w-2xl w-full sm:px-6 lg:px-8">
            <div class="flow-panel p-8">
                <form method="POST" action="{{ route('provider.projects.store') }}" class="space-y-6">
                    @csrf
                    <div>
                    ...
                    <div>
                        <x-input-label for="negotiated_price" :value="__('Negotiated price (:cur)', ['cur' => $provider->company?->default_currency ?? 'USD'])" />
                        <x-text-input id="negotiated_price" name="negotiated_price" type="text" inputmode="decimal" class="mt-1 block w-full flowdesk-amount" value="{{ old('negotiated_price') }}" />
                        <x-input-error :messages="$errors->get('negotiated_price')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="final_deadline" :value="__('Final deadline')" />
                        <input id="final_deadline" type="date" name="final_deadline" value="{{ old('final_deadline') }}" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100" />
                        <x-input-error :messages="$errors->get('final_deadline')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="description" :value="__('Description')" />
                        <textarea id="description" name="description" class="mt-2 block w-full">{{ old('description') }}</textarea>
                    </div>
                    <div class="flex gap-3">
                        <x-primary-button type="submit">{{ __('Create') }}</x-primary-button>
                        <a href="{{ route('provider.projects.index') }}" class="inline-flex items-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 shadow-sm hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200">{{ __('Cancel') }}</a>
                    </div>
                </form>
            </div>
        </div>

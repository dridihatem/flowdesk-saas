<x-app-layout>
    <div class="py-10">
        <div class="max-w-4xl w-full sm:px-6 lg:px-8">
            ...
            <x-flow.page-header
                :title="__('email_marketing_workspace_template_create')"
                :description="__('email_marketing_workspace_template_create_intro')"
            />
            <div class="mt-8 flow-panel p-6 sm:p-8">
                <form method="post" action="{{ route('email-marketing.templates.store') }}" class="space-y-6">
                    @csrf
                    @include('email-marketing.templates._workspace-form', ['template' => null, 'aiAvailable' => $aiAvailable ?? false])

<x-app-layout>
    <div class="py-10">
        <div class="max-w-3xl w-full sm:px-6 lg:px-8">
            ...
            <x-flow.page-header
                :title="__('email_marketing_audience_edit')"
                :description="__('email_marketing_audience_edit_intro')"
            />
            <div class="mt-8 flow-panel p-6 sm:p-8">
                <form method="post" action="{{ route('email-marketing.audiences.update', $audience) }}" class="space-y-6">
                    @csrf
                    @method('put')
                    <div>
                        <x-input-label for="name" :value="__('Name')" />
                        <x-text-input id="name" name="name" class="mt-1 block w-full" :value="old('name', $audience->name)" required />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>
                    <div>
                        <x-input-label for="description" :value="__('Description')" />
                        <textarea id="description" name="description" rows="2" class="mt-1 block w-full rounded-lg border-slate-300 shadow-sm dark:border-slate-600 dark:bg-slate-900 dark:text-white">{{ old('description', $audience->description) }}</textarea>
                    ...
                    <div x-data="flowdeskAudienceContacts({
                        ...
                        >{{ old('contacts_input', $contactsInput) }}</textarea>
                ...
                <div class="mt-10 border-t border-slate-200 pt-8 dark:border-slate-700">
                    <form method="post" action="{{ route('email-marketing.audiences.destroy', $audience) }}" onsubmit="return confirm(@json(__('email_marketing_audience_delete_confirm')))">
                        @csrf
                        @method('delete')
                        <button type="submit" class="text-sm font-semibold text-rose-600 hover:text-rose-500 dark:text-rose-400">{{ __('Remove audience') }}</button>
                    </form>
                </div>
            </div>
        </div>

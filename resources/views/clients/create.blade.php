<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
            <div>
                <h2 class="text-lg font-semibold tracking-tight text-slate-900 dark:text-white">{{ __('New client') }}</h2>
                <p class="text-sm text-slate-500 dark:text-slate-400">{{ __('Add a client to your workspace.') }}</p>
            </div>
            ...
            <div class="flow-panel p-6 sm:p-8">
                <form method="POST" action="{{ route('clients.store') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <x-text-input id="client_name" name="name" type="text" class="mt-2 block w-full" :value="old('name')" required />
                        <x-input-error :messages="$errors->get('name')" class="mt-2" />
                    </div>
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            <x-input-label for="client_email" :value="__('Email')" />
                            <x-text-input id="client_email" name="email" type="email" class="mt-2 block w-full" :value="old('email')" />
                            <x-input-error :messages="$errors->get('email')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="client_phone" :value="__('Phone')" />
                            <x-text-input id="client_phone" name="phone" type="text" class="mt-2 block w-full" :value="old('phone')" />
                    ...
                    <div>
                        ...
                        <select id="client_status" name="status" class="flow-input mt-2 block w-full">
                            @foreach (\App\Enums\ClientStatus::cases() as $statusOption)
                                <option value="{{ $statusOption->value }}" @selected(old('status', \App\Enums\ClientStatus::Active->value) === $statusOption->value)>{{ $statusOption->label() }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="client_source" name="source" class="flow-input mt-2 block w-full">
                            ...
                            @foreach (\App\Enums\ClientSource::cases() as $sourceCase)
                                <option value="{{ $sourceCase->value }}" @selected(old('source') === $sourceCase->value)>{{ $sourceCase->label() }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <x-text-input id="client_line1" name="address_line1" type="text" class="mt-2 block w-full" :value="old('address_line1')" />
                    </div>
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div>
                            <x-input-label for="client_city" :value="__('City')" />
                            <x-text-input id="client_city" name="address_city" type="text" class="mt-2 block w-full" :value="old('address_city')" />
                        </div>
                        <div>
                            <x-input-label for="client_country" :value="__('Country (ISO)')" />
                            <x-text-input id="client_country" name="address_country" type="text" class="mt-2 block w-full uppercase" maxlength="8" :value="old('address_country')" placeholder="US, FR…" />
                        </div>
                    </div>
                    <div class="rounded-xl border border-slate-200 bg-slate-50/60 p-4 dark:border-slate-700 dark:bg-slate-800/40" x-data="{ pwd: '{{ old('portal_password', '') }}' }">
                        ...
                        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">{{ __('client_portal_access_help') }}</p>
                        <div class="mt-3">
                    ...
                    <div class="flex justify-end gap-3 pt-2">
                        ...
                        <x-primary-button type="submit" class="!normal-case">{{ __('Save client') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>

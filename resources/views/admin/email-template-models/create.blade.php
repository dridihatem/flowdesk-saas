<x-admin-layout :title="__('admin_email_template_model_create')">
    <div class="mb-6">
        <a
    ...
    <x-flow.page-header
        :title="__('admin_email_template_model_create')"
        :description="__('admin_email_template_model_create_intro')"
    />

    <div class="flow-panel mt-8 max-w-4xl p-8">
        <form method="POST" action="{{ route('admin.email-template-models.store') }}" class="space-y-6">
            @csrf
            @include('admin.email-template-models._form', ['model' => null])
